...
| [`diffusion_flower.py`](diffusion_flower.py) | the diffusion counterpart on the same photo: the noising trajectory x_0→x_1, then one denoising step made explicit (noised image + t IN → predicted noise, same shape, OUT), looped ×T. Contrasts AR | [svg](../out/examples/diffusion_flower.svg) |
