...
colors = {
    ...
    "nn": ("#A7F3D0", "noise → noise   (fid_kv = fid_q): bidirectional within own chunk"),
    "nc": ("#BAE6FD", "noise → clean   (fid_kv < fid_q): denoise from strictly-past context"),
}
...
right = VStack([
    ...
    Swatches([(colors["cc"][:2] if False else ("#FDE68A", "#CA8A04"), colors["cc"][1]),
              (("#BAE6FD", "#0284C7"), colors["nc"][1]),
              (("#A7F3D0", "#059669"), colors["nn"][1])],
             max_w=380, gap=12),
